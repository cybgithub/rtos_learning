...
*************************************************************************
*/
#define portINITIAL_XPSR			        (0x01000000) //PSR寄存器的bit24 必须置 1，即 0x01000000
...
StackType_t *pxPortInitialiseStack(StackType_t *pxTopOfStack, TaskFunction_t pxCode, void *pvParameters)
{
    ...
    pxTopOfStack--; //指针移动 4 字节
    *pxTopOfStack = portINITIAL_XPSR;                               /* xPSR的bit24必须置1 */
    pxTopOfStack--; //指针移动 4 字节
    *pxTopOfStack = ((StackType_t)pxCode) & portSTART_ADDRESS_MASK; /* R15(PC)， 即函数的入口地址 */
    pxTopOfStack--; //指针移动 4 字节
    *pxTopOfStack = (StackType_t)prvTaskExitError; /* R14(LR)， 即函数的返回地址 */
    pxTopOfStack -= 5;                             /* R12 R3 R2 R1 默认初始化0 */
    *pxTopOfStack = (StackType_t)pvParameters;     /* R0 任务形参 */
    
    /*异常发生时，手动加载到 CPU 寄存器的内容 */
    pxTopOfStack -= 8;                             /* R11 R10 R9 R8 R6 R5 R4 默认初始化0 */
...
/*
 * SVC（Supervisor Call）指令用于产生一个SVC异常
 * 它是用户模式代码中的主进程，用于创造对特权操作系统代码的调用
 * SVC是用于呼叫操作系统所提供API的正道。用户程序只需知道传递给操作系统的参数，而不必知道各API函数的地址
 */
